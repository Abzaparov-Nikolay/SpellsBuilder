...
public class DamageTextSpawner : NetworkBehaviour
{
    ...
    {
        if (!base.IsServer) return;
        SpawnClientRpc(damage, pos, velocity);
    ...
    [ClientRpc]
    private void SpawnClientRpc(float damage, Vector3 pos, Vector3 velocity)
    {
        var newText = Instantiate(damageTextPrefab, pos, Quaternion.identity);
        var data = new DamageTextData(damage, Color.white, velocity);
